...
function render_tournament_overview_plus(s, container, event) {
	...
	event.courts.forEach(function(court) {
		...
		if (!match) {
			var upcoming_matches = event.matches.filter(function(match) {
				if (typeof match.team1_won === 'boolean') {
					return false;
				}

				return match.setup.court_id === court.court_id;
			});
		...
		uiu.el(tr, 'td', 'd_to_court', (isUpcoming ? "*" : "") + (court.label || compat.courtnum(court.id)));
...
function render_tournament_overview_new(s, container, event) {
    ...
    not_announced_matches.sort(function(m1, m2) {
		return courtIdToLabel[m1.setup.court_id].label - courtIdToLabel[m2.setup.court_id].label;
	});
...
function render_dsm2018(s, container, event, court, match, colors) {
	var nscore = extract_netscore(match);
	var setup = match.setup;
	...
	function _render_players(team_id) {
		var tr = uiu.el(table, 'tr', { style: 'height: 11%;' });
		var td = uiu.el(tr, 'td', {});
	...
	var inner_table = uiu.el(score_td, 'table', {
		style: 'border-collapse:collapse;table-layout:fixed;width:100%;height:95%;',
	});
	var border = 1;
		...
		for (var game_id = 0;game_id < max_game_count;game_id++) {
			...
			uiu.el(tr, 'td', {
				style: (
					...
					'background:' + colors.bg4 + ';' +
					'border-right:' + border_str + ' solid ' + colors.bg + ';' +
					'font-size:'+ (document.documentElement.clientWidth * 20 < document.documentElement.clientHeight * 32 ? '20vw' : '32vh') +';font-weight:bold;' +
					'min-width:' + width_str + ';' +
					'max-width:' + width_str + ';' +
...
function render_dsm2018_small(s, container, event, court, match, colors) {
	var nscore = extract_netscore(match);
	var setup = match.setup;
	var max_game_count = calc.max_game_count(setup.counting);
	var team_names = event.team_names || ['', ''];
		...
		for (var game_id = 0;game_id < max_game_count;game_id++) {
			...
			uiu.el(tr, 'td', {
				style: (
					...
					'background:' + colors.bg4 + ';' +
					'border-right:' + border_str + ' solid ' + colors.bg + ';' +
					'font-size:'+ (document.documentElement.clientWidth * 18 < document.documentElement.clientHeight * 30 ? '18vw' : '30vh') +';font-weight:bold;' +
					'min-width:' + width_str + ';' +
					'max-width:' + width_str + ';' +
					(cur_serve ? 'color:' + colors.serv + ';' : '')
				),
...
function update(err, s, event) {
	...
	var xfunc = {
		dsm2018: render_dsm2018,
        'dsm2018-small': render_dsm2018_small,
		andre: render_andre,
	...
	var ofunc = {
        ...
        tournament_overview_plus: render_tournament_overview_plus,
		tim: render_tim,
...
function option_applies(style_id, option_name) {
	var BY_STYLE = {
        ...
        dsm2018: ['court_id', 'cbg', 'team_colors', 'c0', 'c1', 'cfg', 'cfgdark', 'cbg4', 'cserv'],
        'dsm2018-small': ['court_id', 'cbg', 'team_colors', 'c0', 'c1', 'cfg', 'cfgdark', 'cbg4', 'cserv'],
        ...
        tournament_overview_plus: ['cfg', 'cbg', 'cbg3', 'cborder', 'cfg2'],
